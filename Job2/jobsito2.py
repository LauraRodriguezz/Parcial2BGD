...
def lambda2():
    ...
    client.put_object(Body=s.encode('utf-8'), Bucket='headlines-final021', Key=f"periodico=ElTiempo/year={datetime.datetime.now().strftime('%Y')}/month={datetime.datetime.now().strftime('%m')}/day={datetime.datetime.now().strftime('%d')}.csv")
...
def lambda3():
    ...
    # Save the CSV file to an S3 bucket
    client = boto3.client('s3')
    client.put_object(Body=s.encode('utf-8'), Bucket='headlines-final021', Key=f"periodico=ElEspectador/year={datetime.datetime.now().strftime('%Y')}/month={datetime.datetime.now().strftime('%m')}/day={datetime.datetime.now().strftime('%d')}.csv")
